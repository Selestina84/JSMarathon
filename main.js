window.addEventListener('DOMContentLoaded', function () {
 ...
 const logs = {
  ...
  function generateLogs(type, player1, player2, player, value){
    let date = new Date();
    let timeNow = `${date.getHours()}:${date.getMinutes()}`;
  
    switch (type){
      case 'start':
        insertChat(logs[type].replace('[time]', timeNow).replace('[player1]', player1.name).replace('[player2]', player2.name)); 
        break;
      case 'hit': 
        insertChat(`${timeNow} ${logs[type][getRandom(logs.hit.length)-1].replace('[playerKick]', player1.name).replace('[playerDefence]', player2.name)} -${value} [${player.hp}/100]`);
        break;
      case 'defence':
        insertChat(`${timeNow} ${logs[type][getRandom(logs.defence.length)-1].replace('[playerKick]', player1.name).replace('[playerDefence]', player2.name)} `);
        break;
      case 'draw':
